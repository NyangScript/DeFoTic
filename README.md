...
자동으로 가져와 LLM(Gemini) 멀티모달 분석으로 틱 유형·강도·트리거·상황 맥락을 기록합니다.
...
[모바일 앱]  Expo(React Native) — 환자용
   ...
   ├─ Gemini 멀티모달 분석 (CBIT 기능 평가: ABC + 전구 신호 + 경쟁 반응)
...
| `services/` | BLE 관리, 미디어 동기화, Gemini 분석, Firebase, 미디어 파서 |
| `stores/` | 전역 상태 (zustand) — 이벤트/기기 상태 |
| `components/` | 이벤트 카드, 프레임 뷰어, 의료진 웹 뷰어 등 |
| `docs/` | 설계 노트 (펌웨어 설계, LLM 마이그레이션, LoRA 학습 가이드) |
...
```bash
npm install
# Gemini API 키 설정 (.env — 커밋되지 않음)
echo "EXPO_PUBLIC_GEMINI_API_KEY=<발급받은 키>" > .env
...
4. 가져온 이벤트마다 Gemini 멀티모달 분석 → 분석 카드/통계/의료진 공유